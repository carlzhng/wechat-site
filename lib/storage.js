...
function requireBlobOnVercel() {
  if (isVercelRuntime() && !useBlobStorage()) {
    throw new Error(
      'Vercel Blob is not connected. In your Vercel project go to Storage → Blob, connect a store to this project, then redeploy. Set env vars for Production (and Preview if you test branch URLs).'
    );
  }
}
